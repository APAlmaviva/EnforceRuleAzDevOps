...
var propertyExclusion = tl.getInput('propertyExclusion', false); 
var fail = tl.getInput('fail', false); 

var taskVersion="1.0.5"
var agentWF=tl.getVariable("Agent.WorkFolder") + '/_tasks/PropertiesExclusionVerify_d57ab29a-be69-431f-8523-e4bd1b170ec3/'+ taskVersion;
...
'            <configuration>'+
'              <rules>'+
'                <myCustomRule implementation="it.almaviva.enforcer.rule.PropertyExclusionRule">'+
'				   <pomRelativePath>'+path.relative(tl.cwd(),mavenPOMFile)+'</pomRelativePath>'+
'                  <propertyExclusion>'+propertyExclusion+'</propertyExclusion>'+
'                </myCustomRule>'+
'			  </rules>'+
'            </configuration>'+
'          </execution>'+
